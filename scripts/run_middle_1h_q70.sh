...
# Render exactly one hour (216,000 frames at 60 fps) from the middle of the
# 964,389-block source using hardware HEVC at VideoToolbox quality 70.
set -uo pipefail

WORKSPACE="/Users/t/Documents/Kode/Workspaces/Clone UTXO visualizer"
RENDER_DIR="/Volumes/4T Data/buv_render"
RUN_TAG="4k_middle_374195_590194_60fps_q70_epsfix"
VIDEO_OUT="$RENDER_DIR/utxo_${RUN_TAG}.mkv"
...
for path in "$VIDEO_OUT" "$FFMPEG_LOG" "$RENDER_LOG" "$SUPERVISOR_LOG"; do
    if [[ -e "$path" ]]; then
...
: > "$FFMPEG_LOG"
: > "$RENDER_LOG"
record "starting ffmpeg listener: hevc_videotoolbox q70, yuv420p, 4K60"
...
ffmpeg -nostdin -hide_banner -stats_period 30 -y \
    ...
    -c:v hevc_videotoolbox -q:v 70 -tag:v hvc1 -pix_fmt yuv420p \
...
record "starting visualizer for blocks 374195 through 590194"
cd "$WORKSPACE" || exit 1
/usr/bin/caffeinate -i ./build_local/buv -ns -tc=visualizer \
    -cfg=configs/buv_middle_1h_4k_q70.json \
