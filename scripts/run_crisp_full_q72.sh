...
# Supervise the full 4K render using the production pipeline at VideoToolbox
# quality 72. Existing artifacts are never overwritten.
set -uo pipefail

WORKSPACE="/Users/t/Documents/Kode/Workspaces/Clone UTXO visualizer"
RENDER_DIR="/Volumes/4T Data/buv_render"
RUN_TAG="4k_weighted_topband_flashvaluefix_crisp_q72"
VIDEO_OUT="$RENDER_DIR/utxo_4k_epoch105k_60fps_exactledger_weighted_topband_flashvaluefix_crisp_q72.mkv"
...
: > "$FFMPEG_LOG"
: > "$RENDER_LOG"
record "starting ffmpeg listener at VideoToolbox quality 72"

ffmpeg -y -f rawvideo -pixel_format rgb24 -video_size 3840x2160 \
    -framerate 60 -i 'tcp://127.0.0.1:12987?listen' \
    -c:v hevc_videotoolbox -q:v 72 -tag:v hvc1 -pix_fmt yuv420p \
